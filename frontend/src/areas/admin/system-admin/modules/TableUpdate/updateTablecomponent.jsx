...
const UpdateTableComponent = () => {
  ...
  const fetchTables = async () => {
    try {
      ...
      if (response.data.status && response.data.data.get_all_collections) {
        const tables = response.data.data.get_all_collections.map(
          (collection) => ({
  ...
  const confirmDeleteColumn = async () => {
    try {
      await axios.post(
        'http://localhost:3000/api/collection/attribute/delete',
        {
      ...
      {editTable ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {paginatedTables.map((table) => (
            <div
              key={table.name}
              className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 mb-6"
            >
                ...
                <ul className="space-y-2">
                  {table.columns.map((column, index) => (
                    ...
                    >
                      ...
                      <button
                        onClick={() =>
                          handleDeleteColumn(table.name, column.name)
                        }
                ...
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="mt-4 flex justify-end space-x-3">
                    ...
                    <button
                      ...
                      className="bg-red-100 px-4 py-2 rounded text-sm font-medium text-red-900 hover:bg-red-200"
                    >
                      Delete
                    </button>
                  </div>
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="mt-2">
                    <p className="text-sm text-gray-500">
                      Are you sure you want to delete column {selectedColumn}{' '}
